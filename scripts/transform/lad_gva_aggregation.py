#!/usr/bin/env python3
"""
LAD GVA Aggregation Script (Production v1.0)

Takes LAD-level GVA data and aggregates up to ITL3, ITL2, ITL1 levels.
Creates consistent bottom-up hierarchy for GVA forecasting.

Inputs:
- silver.lad_gva_history (from lad_gva_ingest.py)

Outputs:
- silver.itl3_gva_history
- silver.itl2_gva_history  
- silver.itl1_gva_history (bottom-up)
...
log = logging.getLogger("gva_aggregation")
...
def load_lad_data() -> pd.DataFrame:
    """Load LAD GVA data from DuckDB or CSV"""
    if HAVE_DUCKDB and DUCK_PATH.exists():
        log.info("Loading LAD GVA from DuckDB...")
        con = duckdb.connect(str(DUCK_PATH), read_only=True)
        try:
            df = con.execute("SELECT * FROM silver.lad_gva_history").fetchdf()
        ...
        # Fallback to CSV
        csv_path = SILVER_DIR / "lad_gva_history.csv"
        if not csv_path.exists():
            raise FileNotFoundError(f"LAD GVA data not found: {csv_path}")
        
        log.info(f"Loading LAD GVA from CSV: {csv_path}")
...
def aggregate_to_level(
    ...
    """
    Aggregate LAD GVA to a higher geography level.
    
    Args:
        df: LAD GVA data with parent geography columns
    ...
    agg['metric_id'] = 'nominal_gva_mn_gbp'
    agg['unit'] = 'GBP_m'
    agg['freq'] = 'A'
...
def compare_with_topdown(bottomup_itl1: pd.DataFrame):
    ...
    if HAVE_DUCKDB and DUCK_PATH.exists():
        try:
            ...
            topdown = con.execute("""
                ...
                WHERE metric_id = 'nominal_gva_mn_gbp'
            """).fetchdf()
            con.close()
            ...
            topdown = topdown_all[topdown_all['metric_id'] == 'nominal_gva_mn_gbp'].copy()
    else:
        # CSV only
        csv_path = SILVER_DIR / "itl1_unified_history.csv"
        if not csv_path.exists():
    ...
    if topdown.empty:
        log.info("No GVA data in top-down ITL1 for comparison")
        return
    
    log.info(f"Loaded {len(topdown)} top-down ITL1 GVA observations")
...
def main():
    log.info("="*70)
    log.info("LAD GVA TO ITL AGGREGATION v1.0")
    log.info("="*70)
    ...
    if missing_cols:
        ...
        log.error("Please re-run lad_gva_ingest.py with the lookup file")
        return
    
    # Verify metric
    if 'metric_id' in lad_data.columns:
        metrics = lad_data['metric_id'].unique()
        if 'nominal_gva_mn_gbp' not in metrics:
            log.error(f"Expected nominal_gva_mn_gbp metric, found: {metrics}")
            return
    
    log.info(f"\nInput: {len(lad_data)} LAD GVA observations")
    ...
    if not itl3_data.empty:
        itl3_csv = SILVER_DIR / "itl3_gva_history.csv"
        itl3_data.to_csv(itl3_csv, index=False)
        log.info(f"✓ Saved CSV → {itl3_csv}")
        
        write_to_duckdb(itl3_data, "itl3_gva_history")
    ...
    if not itl2_data.empty:
        itl2_csv = SILVER_DIR / "itl2_gva_history.csv"
        itl2_data.to_csv(itl2_csv, index=False)
        log.info(f"✓ Saved CSV → {itl2_csv}")
        
        write_to_duckdb(itl2_data, "itl2_gva_history")
    ...
    if not itl1_data.empty:
        itl1_csv = SILVER_DIR / "itl1_gva_bottomup.csv"
        itl1_data.to_csv(itl1_csv, index=False)
        log.info(f"✓ Saved CSV → {itl1_csv}")
        
        write_to_duckdb(itl1_data, "itl1_gva_bottomup")
    ...
    log.info("\n✅ Bottom-up GVA aggregation complete!")
    log.info("="*70)
