#!/usr/bin/env python3
"""
LAD Employment Aggregation Script (Production v1.0)

Takes LAD-level employment data and aggregates up to ITL3, ITL2, ITL1 levels.
Creates consistent bottom-up hierarchy for employment forecasting.

Inputs:
- silver.lad_employment_history (from lad_emp_ingest.py)

Outputs:
- silver.itl3_employment_history
- silver.itl2_employment_history  
- silver.itl1_employment_history (bottom-up)
...
log = logging.getLogger("emp_aggregation")
...
def load_lad_data() -> pd.DataFrame:
    """Load LAD employment data from DuckDB or CSV"""
    if HAVE_DUCKDB and DUCK_PATH.exists():
        log.info("Loading LAD employment from DuckDB...")
        con = duckdb.connect(str(DUCK_PATH), read_only=True)
        try:
            df = con.execute("SELECT * FROM silver.lad_employment_history").fetchdf()
        ...
        # Fallback to CSV
        csv_path = SILVER_DIR / "lad_employment_history.csv"
        if not csv_path.exists():
            raise FileNotFoundError(f"LAD employment data not found: {csv_path}")
        
        log.info(f"Loading LAD employment from CSV: {csv_path}")
...
def aggregate_to_level(
    ...
    """
    Aggregate LAD employment to a higher geography level.
    
    Args:
        df: LAD employment data with parent geography columns
...
def compare_with_topdown(bottomup_itl1: pd.DataFrame):
    ...
    if not HAVE_DUCKDB or not DUCK_PATH.exists():
        ...
        log.info("Loading top-down ITL1 from CSV...")
        topdown_all = pd.read_csv(csv_path)
        topdown = topdown_all[topdown_all['metric_id'] == 'emp_total_jobs'].copy()
    else:
        try:
            log.info("\n" + "="*70)
            ...
            topdown = con.execute("""
                ...
                WHERE metric_id = 'emp_total_jobs'
            """).fetchdf()
    ...
    if topdown.empty:
        log.info("No employment data in top-down ITL1 for comparison")
        return
    
    log.info(f"Loaded {len(topdown)} top-down ITL1 employment observations")
...
def main():
    log.info("="*70)
    log.info("LAD EMPLOYMENT TO ITL AGGREGATION v1.0")
    log.info("="*70)
    ...
    if missing_cols:
        ...
        log.error("Please re-run lad_emp_ingest.py with the lookup file")
        return
    
    # Verify metric
    if 'metric_id' in lad_data.columns:
        metrics = lad_data['metric_id'].unique()
        if 'emp_total_jobs' not in metrics:
            log.error(f"Expected emp_total_jobs metric, found: {metrics}")
            return
    
    log.info(f"\nInput: {len(lad_data)} LAD employment observations")
    ...
    if not itl3_data.empty:
        itl3_csv = SILVER_DIR / "itl3_employment_history.csv"
        itl3_data.to_csv(itl3_csv, index=False)
        log.info(f"✓ Saved CSV → {itl3_csv}")
        
        write_to_duckdb(itl3_data, "itl3_employment_history")
    ...
    if not itl2_data.empty:
        itl2_csv = SILVER_DIR / "itl2_employment_history.csv"
        itl2_data.to_csv(itl2_csv, index=False)
        log.info(f"✓ Saved CSV → {itl2_csv}")
        
        write_to_duckdb(itl2_data, "itl2_employment_history")
    ...
    if not itl1_data.empty:
        itl1_csv = SILVER_DIR / "itl1_employment_bottomup.csv"
        itl1_data.to_csv(itl1_csv, index=False)
        log.info(f"✓ Saved CSV → {itl1_csv}")
        
        write_to_duckdb(itl1_data, "itl1_employment_bottomup")
    ...
    log.info("\n✅ Bottom-up employment aggregation complete!")
    log.info("="*70)
