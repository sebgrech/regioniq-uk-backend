#!/usr/bin/env python3
"""
LAD to ITL Aggregation Script (Production v1.1)

Takes LAD-level population data and aggregates up to ITL3, ITL2, ITL1 levels.
This creates a consistent bottom-up hierarchy for forecasting.

Inputs:
- silver.lad_population_history (from lad_pop_ingest.py)

Outputs:
- silver.itl3_population_history
- silver.itl2_population_history  
- silver.itl1_population_history (bottom-up)
...
log = logging.getLogger("lad_aggregation")
...
def load_lad_data() -> pd.DataFrame:
    """Load LAD population data from DuckDB or CSV"""
    if HAVE_DUCKDB and DUCK_PATH.exists():
        log.info("Loading LAD data from DuckDB...")
        con = duckdb.connect(str(DUCK_PATH), read_only=True)
        try:
            df = con.execute("SELECT * FROM silver.lad_population_history").fetchdf()
        ...
        # Fallback to CSV
        csv_path = SILVER_DIR / "lad_population_history.csv"
        if not csv_path.exists():
            raise FileNotFoundError(f"LAD data not found: {csv_path}")
        
        log.info(f"Loading LAD data from CSV: {csv_path}")
...
def aggregate_to_level(
    ...
    """
    Aggregate LAD data to a higher geography level.
    
    Args:
        df: LAD data with parent geography columns
        level_code_col: Column name for the target level code (e.g., 'itl3_code')
        level_name_col: Column name for the target level name (e.g., 'itl3_name')
    ...
    agg['metric_id'] = 'population_total'
    agg['unit'] = 'persons'
    agg['freq'] = 'A'
...
def compare_with_topdown(bottomup_itl1: pd.DataFrame):
    ...
    try:
        ...
        log.info(f"Loaded {len(topdown)} top-down ITL1 observations")
        ...
        # Flag regions with large differences
        ...
        if not large_diff.empty:
            ...
            for _, row in worst.iterrows():
                log.warning(
                    ...
                    f"(BU: {row['value_bottomup']:,.0f}, TD: {row['value_topdown']:,.0f})"
                )
            
            log.warning("\nPossible causes:")
            log.warning("  - Boundary changes between LAD and ITL1 vintages")
            log.warning("  - Different NOMIS datasets (LAD vs ITL1 direct)")
            log.warning("  - Data quality issues")
        else:
            log.info("\n✓ Bottom-up and top-down ITL1 are highly consistent (<1% difference)")
        
    except Exception as e:
        log.info(f"Could not compare with top-down ITL1: {e}")
...
def main():
    log.info("="*70)
    log.info("LAD TO ITL AGGREGATION v1.1 (PRODUCTION)")
    log.info("="*70)
    ...
    if missing_cols:
        ...
        log.error("Please re-run lad_pop_ingest.py with the lookup file")
        return
    
    # Quality check: verify we have LAD-level data
    if 'region_level' in lad_data.columns:
        levels = lad_data['region_level'].unique()
        if 'LAD' not in levels:
            log.error(f"Expected LAD-level data, found: {levels}")
            return
    
    log.info(f"\nInput: {len(lad_data)} LAD observations")
    ...
    if not itl3_data.empty:
        # Save CSV
        itl3_csv = SILVER_DIR / "itl3_population_history.csv"
        itl3_data.to_csv(itl3_csv, index=False)
        log.info(f"✓ Saved CSV → {itl3_csv}")
        
        # Save to DuckDB
        write_to_duckdb(itl3_data, "itl3_population_history")
    ...
    if not itl2_data.empty:
        # Save CSV
        itl2_csv = SILVER_DIR / "itl2_population_history.csv"
        itl2_data.to_csv(itl2_csv, index=False)
        log.info(f"✓ Saved CSV → {itl2_csv}")
        
        # Save to DuckDB
        write_to_duckdb(itl2_data, "itl2_population_history")
    ...
    if not itl1_data.empty:
        # Save CSV
        itl1_csv = SILVER_DIR / "itl1_population_bottomup.csv"
        itl1_data.to_csv(itl1_csv, index=False)
        log.info(f"✓ Saved CSV → {itl1_csv}")
        
        # Save to DuckDB
        write_to_duckdb(itl1_data, "itl1_population_bottomup")
    ...
    log.info("\n✅ Bottom-up aggregation complete!")
    log.info("="*70)
