#!/usr/bin/env python3
"""
LAD GVA Ingest from NOMIS API → DuckDB + CSV (Production v1.1 - DEDUP FIX)

Fetches Local Authority District (LAD) level GVA (nominal) data from NOMIS.
Handles ALL boundary changes 2019-2023 with complete concordance mapping.
FIXED: Handles NOMIS API bug that returns duplicate LAD-period rows.

Dataset: NM_2400_1 (Regional GVA)
- Current price estimates (£ million)
- LAD level (cell=0 means total GVA, not by industry)
...
Outputs:
- data/raw/gva/lad_gva_nomis.csv (raw)
- data/silver/lad_gva_history.csv (tidy, 2023 boundaries)
- warehouse.duckdb:
    bronze.gva_lad_raw
...
# Configuration
# -----------------------------
RAW_DIR = Path("data/raw/gva")
RAW_DIR.mkdir(parents=True, exist_ok=True)

SILVER_DIR = Path("data/silver")
SILVER_DIR.mkdir(parents=True, exist_ok=True)
SILVER_CSV = SILVER_DIR / "lad_gva_history.csv"
...
# NOMIS API URL for LAD GVA (nominal, £m)
NOMIS_URL = (
...
log = logging.getLogger("lad_gva_ingest")
...
# COMPLETE LAD CONCORDANCE (same as employment)
LAD_CONCORDANCE = {
...
def fetch_nomis_csv(url: str) -> pd.DataFrame:
    ...
    log.info(f"NOMIS → Fetching LAD GVA data...")
...
def transform_to_tidy(df_raw: pd.DataFrame, lookup: pd.DataFrame) -> pd.DataFrame:
    """
    Transform NOMIS LAD GVA data to tidy schema.
    FIXED: Deduplicates NOMIS API bug that returns duplicate rows.
    ...
    # FIX: DEDUPLICATE NOMIS API BUG
    # NOMIS returns duplicate rows with same LAD-period-value
    ...
    if before_dedup > after_dedup:
        ...
        log.warning(f"⚠️  NOMIS API BUG: Dropped {dropped} duplicate LAD-period records")
        log.warning(f"  This is a known issue with NM_2400_1 dataset")
    
    log.info(f"After deduplication: {initial_lad_count} LADs, {len(tidy)} observations")
    ...
    if post_concordance_lads != initial_lad_count or post_concordance_obs < len(tidy):
        log.info(f"After concordance: {post_concordance_lads} LADs, {post_concordance_obs} observations")
    
    # Final duplicate check (should not exist after dedup + aggregation)
...
def main():
    log.info("="*70)
    log.info("LAD GVA INGEST v1.1 (DEDUPLICATION FIX)")
    log.info("="*70)
    log.info(f"Vintage: {VINTAGE}")
    log.info(f"Dataset: NM_2400_1 (Regional GVA, current prices)")
    log.info(f"Metric: Nominal GVA (£ million)")
    log.info(f"Fix: Handles NOMIS API duplicate row bug")
    log.info(f"\nBoundary changes handled:")
    ...
    # Save raw
    raw_path = RAW_DIR / "lad_gva_nomis.csv"
    df_raw.to_csv(raw_path, index=False)
    log.info(f"✓ Saved raw → {raw_path}")
    
    _write_duck("bronze.gva_lad_raw", df_raw)
    ...
    if HAVE_DUCKDB:
        _write_duck("silver.lad_gva_history", tidy)
    ...
    # Check for years with incomplete coverage
    max_lads = year_coverage.max()
    incomplete_years = year_coverage[year_coverage < max_lads]
    if len(incomplete_years) > 0:
        log.warning(f"⚠️  {len(incomplete_years)} years with incomplete LAD coverage")
    else:
        log.info("✓ All years have complete LAD coverage")
