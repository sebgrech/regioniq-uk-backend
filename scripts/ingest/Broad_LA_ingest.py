#!/usr/bin/env python3
"""
LAD Unified Ingest Pipeline (Production v1.3 - Dedup Fix)
...
- Population (persons): Mid-year estimates
- Employment Rate (%): APS 16-64, fiscal year format
- Unemployment Rate (%): APS model-based, fiscal year format

All metrics:
- Standardized to 2023 LAD boundaries
- Enriched with parent geographies (ITL3→ITL2→ITL1)
- Validated for duplicates and completeness
- Written to bronze (raw) and silver (tidy) layers

v1.3 Changes:
- Added needs_dedup=True to ALL metrics (NOMIS API returns duplicate rows)
...
# Geography ranges for NOMIS API (all LADs, 2023 boundaries)
LAD_GEO_RANGE = (
...
# APS date range (quarterly offsets for annual data)
APS_DATE_RANGE = (
...
# v1.3: ALL metrics now have needs_dedup=True (NOMIS API returns duplicate rows)
METRICS = {
    'employment': {
...
def fetch_nomis_csv(url: str, label: str) -> pd.DataFrame:
...
def transform_to_tidy(
    ...
    # Handle fiscal year format (2005-06 → 2005)
    ...
    # Validate measure if needed (GDHI)
    ...
    # For non-rate metrics, enforce non-negative values
    if not metric_config.get('is_rate', False):
        tidy = tidy[tidy['value'] >= 0]
    
    initial_lad_count = tidy['region_code'].nunique()
    
    # Deduplication - ALWAYS apply (v1.3 fix: NOMIS returns duplicate rows)
    ...
    # Aggregate by LAD-period (old districts → new unitaries)
    # For rate metrics, use mean instead of sum
...
def ingest_employment(lookup: pd.DataFrame) -> pd.DataFrame:
    """
    Employment ingest: Special dual-dataset handling
    2009-2015 (old boundaries) + 2015-2024 (current boundaries)
    """
    log.info("\n" + "="*70)
    ...
    df_raw_2009 = fetch_nomis_csv(config['urls']['2009_2015'], "2009-2015")
    ...
    df_raw_2024 = fetch_nomis_csv(config['urls']['2015_2024'], "2015-2024")
...
def ingest_standard_metric(metric_name: str, lookup: pd.DataFrame) -> pd.DataFrame:
    """Standard single-dataset ingest (GVA, GDHI, population, rates)"""
    log.info("\n" + "="*70)
    ...
    df_raw = fetch_nomis_csv(config['url'], metric_name)
...
def main():
    ...
    log.info("="*70)
    log.info("LAD UNIFIED INGEST PIPELINE v1.3 (DEDUP FIX)")
    log.info("="*70)
    log.info(f"Vintage: {VINTAGE}")
    log.info(f"Metrics: Employment | GVA | GDHI | Population | Emp Rate | Unemp Rate")
    log.info(f"Geography: ~360 LADs (2023 boundaries)")
    log.info(f"\nv1.3 Fix: All metrics now deduplicated (NOMIS API duplicate rows)")
    ...
    try:
        results['employment'] = ingest_employment(lookup)
    ...
    standard_metrics = ['gva', 'gdhi', 'population', 'employment_rate', 'unemployment_rate']
    for metric_name in standard_metrics:
        try:
            results[metric_name] = ingest_standard_metric(metric_name, lookup)
    ...
    log.info("="*70)
    log.info(f"\nNext steps:")
    log.info(f"  1. Review silver CSVs: data/silver/lad_*_history.csv")
    log.info(f"  2. Query DuckDB: silver.lad_*_history tables")
    log.info(f"  3. Proceed to LAD aggregation pipeline")
    log.info("="*70)
