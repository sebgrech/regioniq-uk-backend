#!/usr/bin/env python3
"""
Region IQ - ITL3 Regional Forecasting Engine V3.3
=============================================================================

V3.3: VAR/VECM + Top-Down Reconciliation to ITL2 + Ratio Metric Fix
- Enterprise-grade regional forecasting with cross-metric coherence
- VAR/VECM for GVA ↔ Employment pairs (per ITL3 region)
- Top-down reconciliation to ITL2 parent totals (ADDITIVE METRICS ONLY)
- Ratio metrics (per_head, productivity) recalculated from reconciled components
- Ensures: Σ(ITL3 in ITL2 parent) = ITL2 total, ratios = total / population

Architecture:
  1. Load ITL3 history from silver layer (multi-file bottom-up)
  2. Generate ITL3 regional forecasts using VAR + univariate methods
  3. Reconcile ADDITIVE metrics (GVA, GDHI total, employment, population) to ITL2 parents
  ...
  6. Output to CSV and DuckDB: gold.itl3_forecast

Inputs:
  - data/silver/itl3_population_history.csv
  - data/silver/itl3_employment_history.csv
  - data/silver/itl3_gva_history.csv
  - data/silver/itl3_gdhi_history.csv
  - gold.itl2_forecast (ITL2 forecasts for reconciliation)
  
Outputs:
  - data/forecast/itl3_forecast_v3_long.csv
  - data/forecast/itl3_forecast_v3_wide.csv
  - data/forecast/itl3_confidence_intervals_v3.csv
  - data/forecast/itl3_metadata_v3.json
  - gold.itl3_forecast (DuckDB)

Author: Region IQ
Version: 3.3 (ITL3, Ratio Fix)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('itl3_forecast_engine_v3.log'),
...
@dataclass
class ForecastConfigITL3:
    """V3.3 Configuration for ITL3 with fixed ratio reconciliation"""
    ...
    # Top-down reconciliation (to ITL2)
    use_parent_anchoring: bool = True
    reconciliation_method: str = 'proportional'
    
    # Parent anchor mapping: ITL3 metric → ITL2 metric_id (ADDITIVE ONLY)
    parent_anchor_map: Dict[str, str] = field(default_factory=lambda: {
    ...
    def _validate(self):
        ...
        logger.info("ITL3 V3.3 Configuration validated")
...
class ITL2AnchorManager:
    """Loads ITL2 forecasts for top-down reconciliation"""
    ...
    def _load_anchors(self) -> pd.DataFrame:
        """Load ITL2 forecasts from gold.itl2_forecast"""
        if not HAVE_DUCKDB or not self.duckdb_path.exists():
            logger.warning("Cannot load ITL2 anchors - DuckDB unavailable")
            return pd.DataFrame()
        ...
        try:
            ...
            if anchors.empty:
                logger.warning("No ITL2 forecasts in gold.itl2_forecast")
                return pd.DataFrame()
            ...
            logger.info(
                f"✓ Loaded ITL2 anchors: {anchors['metric'].nunique()} metrics, "
            ...
            logger.error(f"Failed to load ITL2 anchors: {e}")
            return pd.DataFrame()
    ...
    def get_parent_value(
        ...
        """Get ITL2 value for specific metric, year, parent ITL2 code"""
...
# ===============================
# Top-Down Reconciler ITL3 V3.3
# ===============================

class TopDownReconcilerITL3:
    """
    Reconciles ITL3 regional forecasts to ITL2 parent totals.
    V3.3: Fixed to handle ratio metrics correctly.
    
    Process:
      1. Build ITL3→ITL2 parent mapping from lookup file
      2. Reconcile ADDITIVE metrics (totals sum to ITL2 parent totals)
      3. Recalculate RATIO metrics from reconciled components
    """
    
    def __init__(self, config: ForecastConfigITL3, anchor_manager: ITL2AnchorManager):
        self.config = config
        self.anchors = anchor_manager
        self.parent_mapping = self._build_parent_mapping()
    
    def _build_parent_mapping(self) -> pd.DataFrame:
        """Build ITL3→ITL2 parent mapping from lookup file"""
        ...
        try:
            ...
            # Get ITL3→ITL2 mapping
            mapping = lookup[['ITL325CD', 'ITL225CD']].drop_duplicates()
            mapping.columns = ['itl3_code', 'itl2_code']
            
            logger.info(f"✓ Built ITL3→ITL2 mapping: {len(mapping)} ITL3 regions → {mapping['itl2_code'].nunique()} ITL2 parents")
    ...
    def reconcile(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.anchors.has_anchors():
            logger.warning("No ITL2 anchors - skipping reconciliation")
            return data
        
        if self.parent_mapping.empty:
            logger.warning("No parent mapping available - skipping reconciliation")
            return data
        
        # Merge parent codes into forecast data
        ...
        # Drop duplicate itl3_code column
        if 'itl3_code' in data_with_parent.columns:
            data_with_parent = data_with_parent.drop(columns=['itl3_code'])
        
        # Check coverage
        missing_parents = data_with_parent[
            (data_with_parent['data_type'] == 'forecast') &
            (data_with_parent['itl2_code'].isna())
        ]
        
        if not missing_parents.empty:
            missing_regions = missing_parents['region_code'].unique()
            logger.warning(f"⚠️  {len(missing_regions)} ITL3 regions missing parent mapping")
        
        # Use data with parent codes
        data = data_with_parent
        
        logger.info("\n" + "="*70)
        logger.info("TOP-DOWN RECONCILIATION ITL3 → ITL2")
        logger.info("="*70)
        ...
        # STEP 1: Reconcile additive metrics to ITL2
        for metric in forecast_data['metric'].unique():
            itl2_metric = self.config.parent_anchor_map.get(metric)
            if not itl2_metric:
                logger.debug(f"  {metric}: No ITL2 anchor mapping (skipping)")
                continue
            
            logger.info(f"\n  Reconciling {metric} → ITL2 {itl2_metric}")
            
            for parent_region in forecast_data['itl2_code'].dropna().unique():
                for year in forecast_years:
                    year_int = int(year)
                    
                    itl2_value = self.anchors.get_parent_value(
                        itl2_metric, year_int, parent_region
                    )
                    if itl2_value is None:
                        continue
                    ...
                    scale_factor = itl2_value / regional_sum_before
                    ...
                    deviation = (
                        abs(regional_sum_after - itl2_value) / itl2_value
                        if itl2_value > 0 else 0
                    )
                    ...
                    reconciliation_log.append({
                        ...
                        'itl2_metric': itl2_metric,
                        'itl2_value': itl2_value,
                    ...
                    if year_int in [2025, 2030, 2040, 2050]:
                        logger.info(
                            ...
                            f"SF={scale_factor:.4f} | ITL2={itl2_value:,.0f} | "
                            f"ITL3 sum: {regional_sum_before:,.0f}→{regional_sum_after:,.0f}"
                        )
        
        data.attrs['reconciliation_log'] = reconciliation_log
        logger.info(f"\n✓ Additive reconciliation complete: {len(reconciliation_log)} adjustments")
        
        logger.info("\n✓ Ratio metrics handled (GDHI per head will be derived post-forecast)")
        
        return data


# ===============================
# Data Management (ITL3)
# ===============================

class DataManagerITL3:
    """Data management for ITL3 unified silver schema (multi-file)"""
    
    def __init__(self, config: ForecastConfigITL3):
        self.config = config
        
    def load_all_data(self) -> pd.DataFrame:
        """Load ITL3 from unified silver CSVs"""
        cache_key = self._get_cache_key()
        if self.config.cache_enabled and self._cache_exists(cache_key):
            logger.info("Loading ITL3 data from cache")
        ...
        logger.info(f"Loaded ITL3 data: {len(unified)} rows")
        logger.info(
            f"ITL3 regions: {unified['region_code'].nunique()}, "
    ...
    def _load_from_csv_files(self) -> pd.DataFrame:
        """Load all itl3_*_history.csv files"""
        files_to_load = [
            'itl3_population_history.csv',
            'itl3_employment_history.csv',
            'itl3_gva_history.csv',
            'itl3_gdhi_history.csv'
        ]
        ...
        if not dfs:
            raise FileNotFoundError(
                f"No ITL3 history files found in {self.config.silver_dir}"
            )
    ...
    def _handle_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for (region, metric), group in df.groupby(['region_code', 'metric']):
            ...
            if outliers.any():
                logger.info(
                    f"Winsorizing {outliers.sum()} outliers in ITL3 {region}-{metric}"
                )
    ...
    def _get_cache_key(self) -> str:
        hasher = hashlib.md5()
        hasher.update(b"itl3_v3.3")
        return hasher.hexdigest()
    
    def _cache_exists(self, key: str) -> bool:
        return (self.config.cache_dir / f"itl3_data_{key}.pkl").exists()
    
    def _load_from_cache(self, key: str) -> pd.DataFrame:
        with open(self.config.cache_dir / f"itl3_data_{key}.pkl", 'rb') as f:
            return pickle.load(f)
    
    def _save_to_cache(self, df: pd.DataFrame, key: str):
        with open(self.config.cache_dir / f"itl3_data_{key}.pkl", 'wb') as f:
            pickle.dump(df, f)


# ===============================
# VAR System Forecaster (Shared)
# ===============================
...
class VARSystemForecaster:
    ...
    def forecast_system(
        ...
        """Forecast multiple metrics together using VAR/VECM"""
        ...
        try:
            ...
            exog = self._prep_breaks(
                system_df.index, structural_breaks
            ) if structural_breaks else None
            ...
            logger.debug(f"VAR system failed (ITL3 {region_code}): {e}")
            return None
...
class AdvancedForecastingV3:
    """Forecasting engine - shared across all levels"""
    ...
    def _combine_forecasts_properly(
        ...
        """Forecast combination with CV-derived CIs"""
...
class DerivedMetricsV3:
    ...
    def calculate_with_uncertainty(
        ...
        """Monte Carlo with SVD error handling"""
...
# ===============================
# Main Pipeline ITL3 V3.3
# ===============================

class InstitutionalForecasterITL3V3:
    """V3.3 Production pipeline for ITL3"""
    
    def __init__(self, config: ForecastConfigITL3):
        self.config = config
        self.data_manager = DataManagerITL3(config)
        self.forecaster = AdvancedForecastingV3(config)
        self.derived = DerivedMetricsV3()
        
        if config.use_parent_anchoring and HAVE_DUCKDB:
            self.anchor_manager = ITL2AnchorManager(config.duckdb_path)
            if self.anchor_manager.has_anchors():
                self.reconciler = TopDownReconcilerITL3(config, self.anchor_manager)
                logger.info("✓ ITL2 anchors loaded - ITL3→ITL2 reconciliation enabled")
            else:
                self.reconciler = None
                logger.warning("⚠️ No ITL2 anchors - reconciliation disabled")
        else:
            self.anchor_manager = None
            self.reconciler = None
    
    def run(self) -> pd.DataFrame:
        """Execute ITL3 pipeline"""
        logger.info("="*60)
        logger.info("ITL3 FORECASTING ENGINE V3.3")
        logger.info("="*60)
        
        historical = self.data_manager.load_all_data()
        tasks = self._identify_tasks(historical)
        logger.info(f"ITL3 forecasting tasks: {len(tasks)}")
    ...
    def _run_forecasts_with_systems(
        ...
        for region_code, region_tasks in tasks_by_region.items():
            ...
            for system_name, system_metrics in self.config.var_systems.items():
                ...
                if len(available) >= 2 and self.config.use_var_systems and self.forecaster.var_forecaster:
            ...
            for task in region_tasks:
                ...
                try:
                    ...
                    logger.error(f"Failed ITL3 {task['region_code']}-{task['metric']}: {e}")
        ...
        for region_code in data['region_code'].unique():
            ...
            for year in data['year'].unique():
                try:
                    ...
                    logger.error(f"Derived failed ITL3 {region_code}-{year}: {e}")
        return derived
    ...
    def _save_outputs(self, data: pd.DataFrame):
        ...
        metadata = {
            ...
            'config': {
                ...
                'reconciled_to_itl2': self.config.use_parent_anchoring
            },
            'data_summary': {
        ...
        if HAVE_DUCKDB:
            try:
                ...
                data_copy["forecast_version"] = "3.3_itl3"
                ...
                con.close()
                logger.info(f"✓ DuckDB: gold.itl3_forecast ({len(data_flat)} rows)")
...
def main():
    """Run ITL3 forecasting pipeline"""
    
    try:
        config = ForecastConfigITL3()
        forecaster = InstitutionalForecasterITL3V3(config)
        results = forecaster.run()
        
        logger.info("="*70)
        print("\nITL3 FORECAST SUMMARY")
        ...
        print("\nOutputs:")
        print(f"  - data/forecast/itl3_forecast_v3_long.csv")
        print(f"  - data/forecast/itl3_forecast_v3_wide.csv")
        print(f"  - data/forecast/itl3_confidence_intervals_v3.csv")
        print(f"  - data/forecast/itl3_metadata_v3.json")
        print(f"  - gold.itl3_forecast (DuckDB)")
        print("="*70)
