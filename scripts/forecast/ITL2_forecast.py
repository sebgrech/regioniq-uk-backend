...
- Falls back to ensemble when VAR validation fails
...
Architecture remains identical to V3.3:
  1. Load ITL2 history from silver layer (multi-file bottom-up)
  2. Generate ITL2 regional forecasts using VAR + univariate methods
...
Inputs:
  ...
  - data/forecast/forecast_v3_long.csv (ITL1 forecasts for reconciliation)
...
Outputs:
  ...
  - gold.itl2_forecast (DuckDB)

Expected performance:
  - VAR/VECM usage: 10-20% (down from 30-40% in V3.4, but all valid)
  - GVA-Employment correlation: +0.5 to +0.8 (realistic)
  - Zero invalid forecasts (was 14 zero-employment regions in V3.4)
  - Reconciliation: 100% success (was 7 failures in V3.4)
...
class ForecastConfigITL2:
    """V3.4 Configuration for ITL2 with VAR/VECM fixes"""
    ...
    min_history_years: int = 12  # V3.5: Safer middle ground (not 5, not 10)
    ...
    def _validate(self):
        ...
        logger.info(f"ITL2 V3.5 Configuration validated (min_history={self.min_history_years}, safer thresholds + VAR validation)")
...
class ITL1AnchorManager:
    """Loads ITL1 forecasts for top-down reconciliation"""
    ...
    def _load_anchors(self) -> pd.DataFrame:
        """Load ITL1 forecasts from gold.itl1_forecast"""
    ...
    def get_parent_value(
        ...
        """Get ITL1 value for specific metric, year, parent ITL1 code"""
...
class TopDownReconcilerITL2:
    ...
    def _build_parent_mapping(self) -> pd.DataFrame:
        """Build ITL2→ITL1 parent mapping from lookup file"""
        ...
        try:
            ...
            logger.info(f"✓ Built ITL2→ITL1 mapping: {len(mapping)} ITL2 regions → {mapping['itl1_code'].nunique()} ITL1 parents")
    ...
    def reconcile(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Check if we have parent mapping
        ...
        # Drop the duplicate itl2_code column
        ...
        if not missing_parents.empty:
            ...
            logger.warning(f"⚠️  {len(missing_regions)} ITL2 regions missing parent mapping: {missing_regions[:5].tolist()}")
        
        # Use the data with parent codes for reconciliation
        ...
        # STEP 1: Reconcile additive metrics to ITL1
        ...
        # STEP 2: Recalculate ratio metrics (currently just GDHI per head if needed)
        # For now, we only ingest GDHI total, so per head would be calculated here
        # This is where you'd add: gdhi_per_head = gdhi_total * 1e6 / population
...
class DataManagerITL2:
    ...
    def load_all_data(self) -> pd.DataFrame:
        """Load ITL2 from unified silver CSVs"""
    ...
    def _load_from_csv_files(self) -> pd.DataFrame:
        """Load all itl2_*_history.csv files"""
        files_to_load = [
...
class VARSystemForecaster:
    """VAR/VECM for cross-metric coherence - V3.4 with ITL2 fixes"""
    ...
    def forecast_system(
        ...
        try:
            ...
            # ✅ FIX 3: Explicit overlap enforcement
            system_df = pd.DataFrame(series_dict).dropna()
            
            # ✅ FIX 1: Use config.min_history_years (now 5 instead of 10)
            ...
            # V3.5: CRITICAL - Validate VAR results before accepting them
            ...
            for metric, fc in results.items():
                ...
                # Check 2: Negative values (employment/GVA/population can't be negative)
                ...
                # Check 3: Zero collapse in long-term (suspicious)
                if metric in ['emp_total_jobs', 'nominal_gva_mn_gbp']:
                    # Check last 10 forecast years
                    long_term = fc['values'][-min(10, len(fc['values'])):]
                    if (long_term < 1e-6).any():  # Effectively zero
                        logger.warning(f"VAR produced near-zero collapse for {region_code}-{metric}, rejecting")
                        validation_failed = True
                        break
                
                # Check 4: Explosive growth (>25% annual sustained)
                pct_changes = np.diff(fc['values']) / fc['values'][:-1]
                if np.any(pct_changes > 0.25):  # 25% annual growth is implausible
                    logger.warning(f"VAR produced explosive growth for {region_code}-{metric}, rejecting")
                    validation_failed = True
                    break
                
                # Check 5: Unstable confidence intervals (too wide = unreliable)
                ci_width = fc['ci_upper'] - fc['ci_lower']
                cv = ci_width / (2 * np.abs(fc['values']) + 1e-10)  # Coefficient of variation
                if np.any(cv > 2.0):  # CI wider than 200% of point estimate
                    logger.warning(f"VAR produced unstable CIs for {region_code}-{metric}, rejecting")
                    validation_failed = True
                    break
            
            # If validation failed, return None (fall back to univariate)
            if validation_failed:
                logger.info(f"  VAR validation failed for {region_code}, falling back to univariate")
                return None
    ...
    def _test_cointegration(self, data: pd.DataFrame) -> Dict:
        """V3.5: Safer threshold - 15 years for VECM reliability"""
        if len(data) < 15:  # Raised from 8 (too risky) to 15 (middle ground)
...
# ===============================
# Advanced Forecasting (Unchanged from V3.3)
# ===============================

class AdvancedForecastingV3:
    """Forecasting engine - shared between ITL1/ITL2/ITL3"""
...
class DerivedMetricsV3:
    ...
    def calculate_all_derived(
        ...
        # GDHI per head (from reconciled total and population)
...
# ===============================
# Main Pipeline ITL2 V3.4
# ===============================

class InstitutionalForecasterITL2V3:
    """V3.4 Production pipeline for ITL2 with VAR/VECM fixes"""
    ...
    def run(self) -> pd.DataFrame:
        """Execute ITL2 pipeline"""
        logger.info("="*60)
        logger.info("ITL2 FORECASTING ENGINE V3.5 (VAR VALIDATION)")
        logger.info("="*60)
        ...
        # Calculate derived metrics (including GDHI per head)
        ...
        logger.info("✅ ITL2 forecasting V3.4 completed")
        return all_data
    ...
    def _save_outputs(self, data: pd.DataFrame):
        ...
        if HAVE_DUCKDB:
            try:
                ...
                # 4. BULLETPROOF region_level handling
                if "region_level" not in data_copy.columns:
                    # Create and set all to ITL2
                    data_copy["region_level"] = "ITL2"
                else:
                    # Fill any NULLs with ITL2
                    data_copy["region_level"] = data_copy["region_level"].fillna("ITL2")
                
                # Final check: ensure all region_level are ITL2
                data_copy["region_level"] = "ITL2"  # Force all to ITL2 (this is ITL2 pipeline)
                
                # 5. Handle unit mapping
                ...
                # 8. Validation before writing to DuckDB
                null_checks = {
                ...
                if critical_nulls:
                    ...
                    logger.info("✓ All critical columns validated (no NULLs)")
                ...
                # CRITICAL: Unified view for Supabase (history + forecast)
                # This is what the frontend actually queries
                con.execute("""
                ...
                # VERIFICATION: Check for NULLs in gold table
                ...
                if null_name > 0 or null_level > 0 or null_code > 0:
                    logger.error(f"❌ CRITICAL: NULLs detected in gold.itl2_forecast!")
...
def main():
    ...
    try:
        ...
        print(f"  - gold.itl2_forecast (DuckDB)")
        print(f"  - gold.itl2_forecast_only (view)")
        print(f"  - gold.itl2_latest (view: history + forecast)")
        print("="*70)
